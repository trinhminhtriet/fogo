...
pub enum FileHandlingMode {
    ...
    Hide,
    /// Match both files and folders while searching. Note that currently `fogo` doesn't do anything with
...
mod tests {
    ...
    fn check_default_keymap_keys_unique() {
        ...
        for (k, v) in key_counts {
            assert_eq!(
                v, 1,
                "found {} entries for key {:?} in context {:?}",
                v, k.0, k.1
            );
        }
    }
    ...
    fn check_all_actions_have_default_keymap() {
        ...
        let actions_in_default_keymap: Vec<Action> =
            DEFAULT_KEYMAP.iter().map(|(_, _, a)| a.clone()).collect();
        for a in Action::iter() {
            if a != Action::None {
                assert!(
                    actions_in_default_keymap.contains(&a),
                    "Action {:?} not found in default keymap",
                    a
                )
            }
        }
    ...
    fn test_keyboard_mapping_cli_option1() {
        let settings = parse_cli_no_warnings(vec!["foo", "-m", "ctrl-x:Exit"]);
        assert_eq!(
            settings.keymap.get(&(key!(ctrl - x), ActionContext::None)),
            Some(&Action::Exit)
        );
    }
    ...
    fn test_keyboard_mapping_cli_option2() {
        let settings = parse_cli_no_warnings(vec!["foo", "-m", "ctrl-x:Exit,ctrl-y:ClearSearch"]);
        assert_eq!(
            settings.keymap.get(&(key!(ctrl - x), ActionContext::None)),
            Some(&Action::Exit)
        );
        assert_eq!(
            settings.keymap.get(&(key!(ctrl - y), ActionContext::None)),
    ...
    fn test_keyboard_mapping_cli_option3() {
        let settings = parse_cli_no_warnings(vec![
            "foo",
            "-m",
            "ctrl-x:Exit,ctrl-x:ClearSearch", // repeated mapping
        ]);
        assert_eq!(
            settings.keymap.get(&(key!(ctrl - x), ActionContext::None)),
            Some(&Action::ClearSearch)
    ...
    fn test_keyboard_mapping_cli_option_wrong1() {
        let m = crate::cli_args::get_cli_args().get_matches_from(vec![
            "foo",
            "-m",
            "ctrl-x:Exxit", // incorrect action
        ]);
    ...
    fn test_keyboard_mapping_cli_option_wrong3() {
        let m = crate::cli_args::get_cli_args().get_matches_from(vec![
            "foo",
            "-m",
            "ctrl-x:Wrong:Exit", // incorrect context
        ]);
        assert!(TereSettings::parse_cli_args(&m).is_err());
    }
    ...
    fn test_keyboard_mapping_cli_option_wrong5() {
        let m = crate::cli_args::get_cli_args().get_matches_from(vec![
            "foo", "-m", "ctrl-x", // missing mapping and/or context
        ]);
        assert!(TereSettings::parse_cli_args(&m).is_err());
    }
    ...
    fn test_unmap1() {
        ...
        assert_eq!(
            settings.keymap.get(&(key!(alt - h), ActionContext::None)),
            Some(&Action::ChangeDirParent)
        );
        assert_eq!(
            settings.keymap.get(&(key!(alt - j), ActionContext::None)),
            Some(&Action::CursorDown)
        );
        assert_eq!(
            settings.keymap.get(&(key!(alt - k), ActionContext::None)),
            Some(&Action::CursorUp)
        );
        assert_eq!(
            settings.keymap.get(&(key!(alt - l), ActionContext::None)),
            Some(&Action::ChangeDir)
        );

        let settings = parse_cli_no_warnings(vec![
            "foo",
            "-m",
            "alt-h:None,alt-j:None,alt-k:None,alt-l:None",
        ]);
        assert_eq!(
            settings.keymap.get(&(key!(alt - h), ActionContext::None)),
            None
    ...
    fn test_unmap2() {
        ...
        assert_eq!(
            settings
                .keymap
                .get(&(key!(esc), ActionContext::NotSearching)),
            Some(&Action::Exit)
        );
        assert_eq!(
            settings.keymap.get(&(key!(esc), ActionContext::Searching)),
            Some(&Action::ClearSearch)
        );
        ...
        assert_eq!(
            settings
                .keymap
                .get(&(key!(backspace), ActionContext::Searching)),
            Some(&Action::EraseSearchChar)
        );
        assert_eq!(
            settings
                .keymap
                .get(&(key!(backspace), ActionContext::NotSearching)),
            Some(&Action::ChangeDirParent)
        );
        assert_eq!(
            settings.keymap.get(&(key!(backspace), ActionContext::None)),
            None
        );
        ...
        let settings = parse_cli_no_warnings(vec![
            ...
            "esc:Searching:None",
            "-m",
            "backspace:None", // this shouldn't affect any of the mappings since they are context-dependent
            "-m",
            "backspace:None:None", // this shouldn't affect any of the mappings since they are context-dependent
        ]);
        assert_eq!(
            settings
                .keymap
    ...
    fn test_empty_keymap_is_error() {
        let m =
            crate::cli_args::get_cli_args().get_matches_from(vec!["foo", "--clear-default-keymap"]);
        assert!(TereSettings::parse_cli_args(&m).is_err());
    }
    ...
    fn test_unmap_exit_is_error() {
        let m = crate::cli_args::get_cli_args().get_matches_from(vec![
            "foo",
            "--map",
            "esc:NotSearching:None,alt-q:None",
        ]);
        assert!(TereSettings::parse_cli_args(&m).is_err());
    }
    ...
    fn test_filter_search_override() {
        ...
        let settings = parse_cli_no_warnings(vec!["foo", "--filter-search", "--no-filter-search"]);
        assert!(!settings.filter_search);
    ...
    fn test_files_override() {
        ...
        let settings = parse_cli_no_warnings(vec![
            "foo", "--files", "hide", "--files", "ignore", "--files", "h",
        ]);
        assert_eq!(settings.file_handling_mode, FileHandlingMode::Hide);
    }
    ...
    fn test_folders_only_deprecated() {
        ...
        assert!(warnings[0].contains(
            "'--folders-only' / '-d' has been deprecated, please use '--files hide' instead"
        ));
        assert_eq!(settings.file_handling_mode, FileHandlingMode::Hide);
    ...
    fn test_case_sensitive_mode_override() {
        ...
        let settings = parse_cli_no_warnings(vec!["foo", "--case-sensitive", "--ignore-case"]);
    ...
    fn test_gap_search_mode_override() {
        ...
        let settings = parse_cli_no_warnings(vec!["foo", "--gap-search", "--gap-search-anywhere"]);
        ...
        assert_eq!(
            settings.gap_search_mode,
            GapSearchMode::NormalSearchAnywhere
        );
    ...
    fn test_no_gap_search_deprecated() {
        let (settings, warnings) = parse_cli(vec!["foo", "--no-gap-search"]);
        assert_eq!(warnings.len(), 1);
    ...
    fn test_sort_mode_override() {
        ...
        let settings = parse_cli_no_warnings(vec!["foo", "--sort", "created"]);
        assert_eq!(settings.sort_mode, SortMode::Created);

        let settings = parse_cli_no_warnings(vec![
            "foo", "--sort", "created", "--sort", "name", "--sort", "modified",
        ]);
    ...
    fn test_mouse_override() {
        ...
        let settings = parse_cli_no_warnings(vec!["foo", "--mouse", "off", "--mouse", "on"]);
        assert!(settings.mouse_enabled);

        let settings = parse_cli_no_warnings(vec![
            "foo", "--mouse", "off", "--mouse", "on", "--mouse", "off",
        ]);
        assert!(!settings.mouse_enabled);
    }
    ...
    fn test_skip_first_run_prompt() {
        ...
        let settings = parse_cli_no_warnings(vec!["foo", "--skip-first-run-prompt"]);
        assert!(settings.skip_first_run_prompt);
    }
}
